"""
Run inference on all preprocessed myeloma samples using the original oral cancer checkpoint.
Tests on both training and test samples (4 total).
"""
...
def main():
    ...
    myeloma_train_data_path = "/athena/angsd/scratch/ssl4003/deep-learning/multiple_myeloma/processed/myeloma_train_data.pt"
    myeloma_train_labels_path = "/athena/angsd/scratch/ssl4003/deep-learning/multiple_myeloma/processed/myeloma_train_labels.pt"
    myeloma_train_names_path = "/athena/angsd/scratch/ssl4003/deep-learning/multiple_myeloma/processed/myeloma_train_names.txt"
    myeloma_test_data_path = "/athena/angsd/scratch/ssl4003/deep-learning/multiple_myeloma/processed/myeloma_test_data.pt"
    ...
    # Load all myeloma data (train + test)
    print("Loading all myeloma samples (train + test)...")
    X_train = torch.load(myeloma_train_data_path, map_location="cpu").float()
    y_train = torch.load(myeloma_train_labels_path, map_location="cpu").long()
    with open(myeloma_train_names_path) as f:
    ...
    names_all = train_names + test_names
    
    print(f"✓ Loaded {X_all.shape[0]} samples total ({len(train_names)} train + {len(test_names)} test)")
    ...
    print("=" * 70)
    print("ZERO-SHOT EVALUATION: All Myeloma Samples on Oral Cancer Model")
    print("=" * 70)
    print(f"Oral Checkpoint:           {oral_checkpoint_path}")
    print(f"Oral Epoch/Val-PCC:        {epoch} / {val_pearson}")
    print(f"Total Myeloma Samples:     {len(true)} (train={len(train_names)}, test={len(test_names)})")
    ...
    # Per-subset metrics
    train_mask = np.arange(len(names_all)) < len(train_names)
    test_mask = np.arange(len(names_all)) >= len(train_names)
    ...
    for i, (name, pred_label, raw_pred, true_label) in enumerate(
        zip(names_all, pred, raw_preds, true)
    ):
        subset = "TRAIN" if i < len(train_names) else "TEST "
        match = "✓" if pred_label == true_label else "✗"
        print(f"  {i+1}. [{subset}] {match} {name:45s} | Pred={pred_label} Raw={raw_pred:+.4f} | True={true_label}")
