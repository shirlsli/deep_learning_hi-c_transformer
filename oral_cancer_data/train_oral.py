...
def preprocess(hic_dir, out_dir):
    ...
    for fpath in hic_files:
        ...
        #profile = extract_genome_rowsum_profile(fpath)  # genome-wide: 77,405 dims, val_pearson ~0.22 (worse)
...
data, labels = preprocess(
    hic_dir=os.path.join("/athena/angsd/scratch/ssl4003/deep-learning/oral_cancer_data", "HiC"),
    out_dir=os.path.join("/athena/angsd/scratch/ssl4003/deep-learning/oral_cancer_data", "processed"),
)

DATA_PATH         = "processed/oral_train_data.pt"
LABELS_PATH       = "processed/oral_train_labels.pt"
...
input_dim  = data.shape[-1] if data.dim() > 1 else 1
